...
private:
    ...
    /// True for a name that selects a member of a scope or of a value, like the `MEMBER` of
    /// `pkg::MEMBER`, or the `bar` of `foo.bar` and `arr[0].bar`. Those names are not visible
    /// in the enclosing scope, so they need the lookup that hover and goto use.
    bool isMemberName(const parsing::Token& token) const {
        auto* node = m_syntaxes.getTokenParent(&token);
        for (int depth = 0; node && depth < 3; depth++, node = node->parent) {
            switch (node->kind) {
                case syntax::SyntaxKind::ScopedName: {
                    // Both `pkg::MEMBER` and `foo.bar` are ScopedNames, the first with a `::`
                    // separator and the second with a `.`; the member is what follows it
                    auto& scoped = node->as<syntax::ScopedNameSyntax>();
                    return scoped.right->getFirstToken() == token;
                }
                case syntax::SyntaxKind::DotMemberClause:
                    // A member access that is not part of a name, e.g. inside a port header
                    return node->as<syntax::DotMemberClauseSyntax>().member == token;
                case syntax::SyntaxKind::MemberAccessExpression:
                    return node->as<syntax::MemberAccessExpressionSyntax>().name == token;
                default:
                    break;
            }
        }
    ...
    std::optional<TokenClass> classifyReference(const parsing::Token& token) const {
        ...
        // A name that selects a member states where it comes from, so it may not be visible
        // here at all; resolving it is what hover and goto do for the same token
